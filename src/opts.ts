...
/**
 * Reject values that could break out of a Dockerfile instruction line via
 * an embedded newline, which would let the value inject additional
 * Dockerfile instructions.
 */
export function assertSingleLine(value: string, fieldName: string): void {
  if (/[\r\n]/.test(value)) {
    throw new Error(`Invalid value for ${fieldName}: must not contain newlines`);
  }
}

/**
 * Single-quote a value for safe interpolation into a Dockerfile RUN
 * instruction, which Docker executes via `sh -c`.
 */
